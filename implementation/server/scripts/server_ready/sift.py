...
def process_sift(photo_bytes, screen_bytes):
    ...
    # 3. Match features (FlannBased)
    descriptor_matcher = cv2.DescriptorMatcher_create('FlannBased')
    
    # Flann matcher requires float32 descriptors for SIFT
    if des_photo is not None and des_photo.dtype != np.float32:
        des_photo = des_photo.astype(np.float32)
    if des_screen is not None and des_screen.dtype != np.float32:
    ...
    if len(good_matches) < 10:
        raise ImageProcessingError(f"Not enough good matches found (need at least 10 for SIFT, found {len(good_matches)}).")
